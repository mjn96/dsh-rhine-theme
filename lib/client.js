window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = "body{--rhine-ink:" + ink + ";--rhine-paper:" + paper + ";--rhine-surface:" + surface + ";--rhine-conversation:" + conversation + ";--rhine-grid:color-mix(in oklch," + ink + " 3.2%,transparent);--rhine-arc:color-mix(in oklch," + ink + " 8.5%,transparent);--dsw-alias-button-info-fill:" + orange + ";--dsw-alias-button-info-hover:" + orangeHover + ";--dsw-alias-state-business-primary:" + orange + ";--dsw-alias-label-primary-foreground:" + ink + ";--dsw-specific-bubble:oklch(0.955 0.028 67);--dsw-static-deepseek-500:" + ink + ";--dsw-static-deepseek-200:oklch(0.620 0.012 150);--dsw-specific-input-major:" + surface + ";--dsw-alias-border-l4:color-mix(in oklch," + orange + " 55%,oklch(0.8337 0.0069 115.73));--dsw-alias-border-l2-darkmode-thin:oklch(0.8337 0.0069 115.73);--dsw-alias-state-business-tertiary:oklch(0.935 0.045 65);--dsw-alias-label-primary-bluish:oklch(0.360 0.105 48);}" +
			...
			// The hero mark is 148.24px wide but the official grid's icon column is
			// a fixed 34px; overflow must go entirely LEFTWARD (into the page's own
			// margin) or it eats the 10px column-gap and overlaps the headline text.
			// The recenter shift below is the analytic half of that leftward overflow
			// ((148.24 - 34) / 2 = 57.12px) — a pure function of this SVG's own
			// geometry, independent of the headline text/font/badge width, so it
			// never drifts when the upstream copy changes.
			".pXSMma_headline:has([data-rhine-mark=\"hero\"]){transform:translateX(57.12px);}" +
		...
		function HeroInfinityMark(props) {
			...
			// The span itself stays at the official 34px icon-column width; the
			// SVG is absolutely positioned flush against the RIGHT edge, so all of
			// its (width - 34) overflow goes strictly leftward — never into the
			// 10px column-gap that separates it from the headline text (a
			// symmetric overflow here would eat that gap and overlap the text).
			// The headline-level translateX above recenters the whole row by
			// exactly half of that leftward overflow.
			return react.createElement("span", { "data-rhine-mark": "hero", style: { width: size + "px", height: height + "px", position: "relative", display: "inline-flex", alignItems: "center", flex: "0 0 auto", overflow: "visible" } }, InfinitySvg(width, height, props.className, { position: "absolute", right: "0px", top: "50%", transform: "translateY(-50%)" }));
		}
